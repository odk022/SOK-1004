...
# gå til nettsiden over og kopier inn fra nederst på siden
...
# kan også hente xl-fil
#
# kan lage comments for mange linjer ved å bruke shift+ctr+c
# lese data fra xlsx på samme måte
# library(readxl)
...
# $ velger en bestemt kolonne (variabel) i dataene
unique(co2data$country)

# hvor mange country kategorier er det?
length(unique(co2data$country))

# nå kan vi benytte "piping" %>%  - shift+ctr+M
library(tidyverse)

# samme som unique() over, men i tidyverse
# kan leses som: velg co2data,så country, distinct istenfor unique
...
land <-
  co2data %>%
  group_by(country) %>% 
  summarise(n=n()) # n angir navnet på observasjonenen

# sorterer på antall observasjoner
land %>% arrange(n) #- foran n sorterer synkende
...
# bruker mutate når vi skal lage nye variabler
co2data %>%
