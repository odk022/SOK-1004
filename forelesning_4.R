...
rm(list = ls()) # fjerner det som er i environment

# setter arbeidskatalog
getwd()
setwd("~/Studier/SOK-1004/Forelesninger/forelesning_4")
...
# subsetter, ver 1
# rekke 1 til 516 er det magiske intervallet vi er ute etter for å unngå slutten
df_lower <- df_lower[1:516 , ]
rm(df_lower)
...
which(df_lower$Year %in% "Year") #hvor står det "Year" i kolonnen Year
...
df_lower <- df_lower %>% clean_names() # endrer til små bokstaver

# velger de tre første kolonnene
df_lower <- df_lower %>% select(year, mo, globe)

# litt mer data wrangling
library(lubridate)
library(zoo)

# for å lage datovariabel, trenger vi: år - mnd - dagtall
# paste(df_lower$year, df_lower$mo, 1, sep = "-"). Dette limes inn nedenfor.

df_lower <-
  df_lower %>% 
  mutate(dato = ymd(paste(year, mo, 1, sep = "-"))) %>% 
  mutate_if(is.character, ~as.numeric(.)) # hvis chr, så skal den omgjøres til numerisk
df_lower
...
read_table2("https://www.nsstc.uah.edu/data/msu/v6.0/tlt/uahncdc_lt_6.0.txt") %>% 
  .[1:which(.$Year %in% "Year")-1, ] %>% # erstatter navnet på datasettet med "." som er generelt
  clean_names() %>% 
...
# Setter inn linken til de ulike datasettene inn i den generelle funksjonen "scrape"
# endrer navn på globe slik at det tilsvarer datagrunnlaget
...
# Legger inn alle dataene i ett datasett
...
# med gjennomsnitt
dframe %>% 
  mutate(snitt=rowMeans(.[ , -1])) %>% # alle kolonner untatt datokolonnen
  pivot_longer(-dato,
               names_to = "nivå",
               values_to = "temperatur") %>% 
  ggplot(aes(x=dato, y=temperatur, color=nivå)) + geom_line() + theme_bw()
...
dframe2 %>% 
  select(dato, globe, nivå) %>% 
  dplyr::rename(temperatur = globe) %>% #NB "::" bruk dplyr(se linje 249)
  as_tibble()
